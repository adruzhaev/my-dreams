# Start
welcome =
    🌙 *Welcome to your Dream Interpreter*

    Tell me any dream and I'll interpret it through three lenses:
    🧠 Jungian · 💭 Freudian · 🔮 Symbolic

    Just type your dream or send a voice message whenever you're ready.

# Dream flow
interpreting = 🔮 Interpreting your dream...
transcribing = 🎙️ Transcribing your dream...
thinking = 💭 Thinking...
generating-image = 🎨 Generating your dream image...

# New dream
new-dream-button = 🌙 New Dream
new-dream-ready = ✨ Ready for your next dream. Share it whenever you're ready.

# Errors
error-ai = ⚠️ AI error: { $message }
error-unexpected = 😵 Something unexpected happened. Please try again.
error-global = 😵 Something went wrong. Please try again.
error-no-interpretation = Could not interpret dream.
error-no-followup = Could not answer follow-up.
